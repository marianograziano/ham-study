...
async function ensureWasmBinary(): Promise<Uint8Array> {
  ...
  wasmFetchPromise = fetch("/wasm/nec2c.wasm")
    .then((response) => {
      if (!response.ok)
        throw new Error(`Failed to fetch nec2c.wasm: ${response.statusText}`);
...
export class Nec2Context {
  private wires: Nec2Wire[] = [];
  private sources: Nec2Source[] = [];
  private loads: Nec2Load[] = [];
  ...
  set_frequency(mhz: number) {
    this.frequency = mhz;
  }
  set_ground(height_lambda: number) {
    this.groundHeight = height_lambda;
  }
  set_material(mat: string) {
    this.material = mat;
  }
  ...
  async calculate() {
    ...
    const Module = await createNec2Module({
      wasmBinary,
    ...
    try {
      ...
      if (err.name !== "ExitStatus" && err.constructor?.name !== "ExitStatus")
        throw err;
    }

    if (!Module.FS.analyzePath(outputPath).exists)
      throw new Error("NEC2 failed.");
  ...
  private parseResults(data: string) {
    ...
    for (let line of lines) {
      ...
      if (line.includes("RADIATION PATTERNS")) {
        section = "pattern";
        continue;
      }
      if (line.includes("CURRENTS AND LOCATION")) {
        section = "current";
        continue;
      }
      ...
      if (section === "impedance") {
        ...
        if (parts.length >= 8 && !Number.isNaN(Number.parseInt(parts[0], 10))) {
      ...
      if (section === "current") {
        ...
        if (
          parts.length >= 10 &&
          !Number.isNaN(Number.parseInt(parts[0], 10))
        ) {
      ...
      if (section === "pattern") {
        ...
        if (parts.length >= 5 && !Number.isNaN(Number.parseFloat(parts[0]))) {
          ...
          const gain = 10 ** (Math.max(-40, rawGainDbi) / 10);
          this.patternCache.set(
            `${Math.round(parseFloat(parts[0]))},${Math.round(parseFloat(parts[1]))}`,
            gain,
          );
        }
      }
  ...
  get_impedance(tag: number) {
    const cached = this.impedanceCache.get(tag);
    return cached ? [cached.re, cached.im] : [50, 0];
  }
  get_currents() {
    return this.currents;
  }
  get_max_gain() {
    return this.maxGainDbi;
  }
  get_max_gain_direction() {
    return { theta: this.maxGainTheta, phi: this.maxGainPhi };
  }
  get_center() {
    return this.center;
  }
  ...
  get_gain_at(theta_deg: number, phi_deg: number): number {
    const t = Math.round(theta_deg / 5) * 5;
    const p = (Math.round(phi_deg / 5) * 5 + 360) % 360;
    return this.patternCache.get(`${t},${p}`) ?? 1.0;
  }
  ...
  get_max_field_reference(): number {
    ...
    if (maxAmp < 0.0001) {
      for (let t = 0; t <= Math.PI; t += Math.PI / 6) {
        for (let p = 0; p < Math.PI * 2; p += Math.PI / 6) {
          ...
          if (this.groundHeight !== null && this.groundHeight > 0 && tz < 0)
            continue;
          const { amplitude: amp } = this.calculate_field_and_amplitude(
            tx,
            ty,
            tz,
            0,
    ...
    console.log(
      `[NEC2] MaxGain: ${this.maxGainDbi.toFixed(2)}dBi at T=${this.maxGainTheta} P=${this.maxGainPhi}, RefField: ${maxAmp.toFixed(4)}`,
    );
   ...
   */
  calculate_field_and_amplitude(
    x: number,
    y: number,
    z: number,
    time_phase: number,
  ): { instantaneous: number; amplitude: number } {
    let ex_re = 0,
      ex_im = 0;
    let ey_re = 0,
      ey_im = 0;
  ...
  calculate_far_field_pattern_3d(
    thetas: Float64Array,
    phis: Float64Array,
    output: Float64Array,
  ) {
    for (let i = 0; i < thetas.length; i++) {
      const t = Math.min(
        180,
        Math.max(0, Math.round((thetas[i] * 180) / Math.PI / 5) * 5),
      );
