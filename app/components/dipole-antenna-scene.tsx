  ...
  useEffect(() => {
    ...
    const runSimulation = async () => {
      ...
      try {
        ...
        if (!isInvertedV) {
            ctx.add_wire(0, -totalLength/2, h_m, 0, totalLength/2, h_m, 0.001, 19, 1);
  ...
  const LegendPanel = () => (
    <div className="p-4 bg-black/70 text-white rounded-lg md:max-w-xs h-full border border-white/5">
      ...
      <p className="text-xs text-muted-foreground mb-3 leading-relaxed">
        <Trans ns="scene" i18nKey="dipoleAntenna.desc" components={{ br: <br /> }} />
      </p>
          ...
          <div className="pt-2 border-t border-white/5">
            <div className="flex flex-col space-y-2">
              <div className="flex items-center justify-between group">
                <Label htmlFor={`${uniqueId}inv-v-mode`} className="text-[11px] text-zinc-400 cursor-pointer peer-data-[state=checked]:text-white order-first">{t("common.controls.invertedV")}</Label>
                <Switch 
