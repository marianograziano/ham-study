...
import { type BufferGeometry, Color, Matrix4, SphereGeometry, Vector3 } from "three";
...
function MagneticLoopElectricField({
  ...
  const { maxGainLin, localMaxField } = useMemo(() => {
    ...
    let maxV = 1e-9;
    const testPoints = [
        {x: 0, z: 1.5}, {x: 1.2, z: 1.2}, {x: -1.2, z: 1.2}, {x: 0, z: 2.5}
    ];
    for (const pt of testPoints) {
        const { amplitude } = context.calculate_field_and_amplitude(center.x + pt.x, center.y, center.z + pt.z, 0);
        if (amplitude > maxV) maxV = amplitude;
    }
    
    return { 
        maxGainLin: Math.pow(10, maxDbi / 10),
  ...
  useFrame((_state, delta) => {
    ...
    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        ...
        const theta_rad = Math.acos(Math.max(-1, Math.min(1, cY / (r_visual + 1e-6))));
        ...
        const norm = Math.min(1.8, (amplitude * (r_dist_m * 0.5 + 0.2)) / (localMaxField + 1e-12));
        ...
        const visualWeight = Math.pow(Math.max(0, (norm - 0.25) / 0.75), 1.5) * gainWeight * mask;
        const ampWeight = visualWeight * 20.0;

        const phase = timeRef.current - k_wave_visual * r_dist_m;
        const sinPhase = Math.sin(phase);
        const displayWeight = Math.min(1.0, ampWeight * (0.15 + 0.85 * Math.abs(sinPhase)));
...
function MagneticLoopAntenna({ radius = 2, scale = 1 }: { radius?: number; scale?: number }) {
  return (
    <group scale={[scale, scale, scale]}>
      <mesh position={[0, 0, 0]}><torusGeometry args={[radius, 0.05, 16, 100]} /><meshStandardMaterial color="#ef4444" roughness={0.3} metalness={0.8} /></mesh>
      <group position={[0, radius, 0]}><mesh><boxGeometry args={[0.4, 0.4, 0.4]} /><meshStandardMaterial color="#cbd5e1" /></mesh></group>
      <group position={[0, -radius, 0]}><mesh position={[0, radius * 0.2, 0]}><torusGeometry args={[radius * 0.2, 0.03, 16, 50]} /><meshStandardMaterial color="#3b82f6" /></mesh></group>
      <mesh position={[0, -radius - 1, 0]}><cylinderGeometry args={[0.1, 0.1, 2, 16]} /><meshStandardMaterial color="#444" /></mesh>
    </group>
  );
}
...
function RadiationPattern({ context }: { context: Nec2Context | null }) {
  ...
  useEffect(() => {
    ...
    const generateGeometry = () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        thetas[i] = theta; phis[i] = phi;
      }
      context.calculate_far_field_pattern_3d(thetas, phis, gains);
      let maxLinearG = 0.01;
      for (let i = 0; i < count; i++) if (gains[i] > maxLinearG) maxLinearG = gains[i];
      const maxDbi = context.get_max_gain();
  ...
  return (<mesh geometry={geometry}><meshBasicMaterial color="#22c55e" wireframe={true} transparent={true} opacity={0.2} /></mesh>);
}
    ...
    const runSimulation = async () => {
      ...
      try {
        ...
        for (let i = 0; i <= sides; i++) {
          ...
          points.push([radius * Math.cos(angle), 0, radius * Math.sin(angle) + z_offset]);
        }
        for (let i = 0; i < sides; i++) {
          ctx.add_wire(points[i][0], points[i][1], points[i][2], points[i+1][0], points[i+1][1], points[i+1][2], 0.01, 3, i + 1);
        }
        const L_approx = 4e-7 * Math.PI * radius * (Math.log((8 * radius) / 0.01) - 2);
      ...
      } catch (err) { console.error("NEC Error:", err); }
  ...
  const LegendPanel = () => (
    <div className="p-4 bg-black/70 text-white rounded-lg md:max-w-xs h-full border border-white/5">
      ...
      <p className="text-xs text-muted-foreground mb-3 leading-relaxed">
        <Trans ns="scene" i18nKey="magneticLoopAntenna.desc" components={{ br: <br /> }} />
      </p>
      <div className="space-y-1.5 text-xs border-t border-gray-600 pt-2 pb-2">
        <div className="flex items-center gap-2"><div className="w-3 h-3 bg-red-500 rounded-sm" /><span>{t("magneticLoopAntenna.loop")}</span></div>
        <div className="flex items-center gap-2"><div className="w-3 h-3 bg-gray-300 rounded-sm" /><span>{t("magneticLoopAntenna.capacitor")}</span></div>
        <div className="flex items-center gap-2"><div className="w-3 h-3 border-2 border-green-500 rounded-sm" /><span>{t("magneticLoopAntenna.pattern")}</span></div>
      </div>
      ...
      <div className="flex flex-col space-y-4">
        <div className="bg-zinc-900/50 p-3 rounded border border-white/5">
          <div className="grid grid-cols-2 gap-2">
            <div><div className="text-[9px] text-zinc-400 mb-0.5">{t("common.simulation.peakGain")}</div><div className="text-xs font-mono text-green-400">{isCalculating ? "..." : `${maxGain.toFixed(2)} dBi`}</div></div>
            <div><div className="text-[9px] text-zinc-400 mb-0.5">{t("common.simulation.impedance")}</div><div className="text-xs font-mono text-zinc-300">{isCalculating ? "..." : impedance ? `${impedance.re.toFixed(1)}Ω` : "--"}</div></div>
          </div>
        </div>
        <div className="space-y-3">
          <div>
            <div className="mb-2 text-xs font-medium text-zinc-300">{t("common.simulation.material")}</div>
            <RadioGroup value={material} onValueChange={setMaterial} className="flex gap-3">
              <div className="flex items-center space-x-2"><RadioGroupItem value="copper" id={`${uniqueId}m-cu`} className="peer size-3" /><Label htmlFor={`${uniqueId}m-cu`} className="text-[11px] text-zinc-400 peer-data-[state=checked]:text-white">{t("common.simulation.copper")}</Label></div>
              <div className="flex items-center space-x-2"><RadioGroupItem value="aluminum" id={`${uniqueId}m-al`} className="peer size-3" /><Label htmlFor={`${uniqueId}m-al`} className="text-[11px] text-zinc-400 peer-data-[state=checked]:text-white">{t("common.simulation.aluminum")}</Label></div>
            </RadioGroup>
          </div>
          <div className="pt-2 border-t border-white/5">
            <div className="mb-2 text-xs font-medium text-zinc-300">{t("common.simulation.groundHeight")}</div>
            <div className="flex items-center space-x-3">
              <input type="range" min="0" max="2" step="0.1" value={groundHeight} onChange={(e) => setGroundHeight(Number.parseFloat(e.target.value))} className="w-full accent-blue-500 h-1" />
          ...
          <div className="pt-2 border-t border-white/5 flex flex-col space-y-2 pb-2">
            <div className="flex items-center justify-between group"><Label htmlFor={`${uniqueId}wave-mode`} className="text-[11px] text-zinc-400">{t("common.controls.showWaves")}</Label><Switch id={`${uniqueId}wave-mode`} checked={showWaves} onCheckedChange={setShowWaves} className="scale-75" /></div>
            <div className="flex items-center justify-between group"><Label htmlFor={`${uniqueId}pattern-mode`} className="text-[11px] text-zinc-400">{t("common.controls.showPattern")}</Label><Switch id={`${uniqueId}pattern-mode`} checked={showPattern} onCheckedChange={setShowPattern} className="scale-75" /></div>
          </div>
        </div>
        <Button variant="secondary" size="sm" className="w-full h-8 text-[11px]" onClick={handleDownload}><Camera className="mr-2 size-3.5" /> {t("common.controls.download")}</Button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className={`relative w-full ${isThumbnail ? "h-full" : "h-[450px] md:h-[600px]"} border rounded-lg overflow-hidden bg-black touch-none`}>
        <Canvas ref={canvasRef} gl={{ preserveDrawingBuffer: true }} camera={{ position: [6, 4, 8], fov: 45 }} frameloop={isThumbnail && !isHovered ? "demand" : "always"}>
          <color attach="background" args={["#111111"]} /><fog attach="fog" args={["#111111", 100, 1000]} />
          {!isThumbnail && <ArcballControls target={[0, 0, 0]} makeDefault />}
          <ambientLight intensity={0.5} /><directionalLight position={[10, 10, 10]} intensity={1} /><axesHelper args={[5]} />
          ...
          <group position={[0, 0, 0]}>
            ...
            {showWaves && context && <MagneticLoopElectricField context={context} speed={effectiveSpeed} visualScale={visualScale} offsetX={0.0} />}
          </group>
        </Canvas>
        {!isThumbnail && (<><div className="absolute top-4 left-4 pointer-events-none select-none md:block hidden"><LegendPanel /></div><div className="absolute bottom-4 right-4 pointer-events-auto w-64 max-h-[85%] overflow-y-auto md:block hidden"><ControlsPanel /></div></>)}
      </div>
      {!isThumbnail && <div className="flex flex-col gap-4 md:hidden px-4 pb-4"><LegendPanel /><ControlsPanel /></div>}
    </div>
  );
}
